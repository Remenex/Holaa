...
export default function BestMovieWidget({ movie, token }: Props) {
  return (
    <div className="flex flex-col items-center max-w-[355px]">
      ...
      <div className="relative group w-full h-[370px] overflow-hidden rounded-[40px] border-[5px] border-white">
        <Image
          src={`${token ? process.env.NEXT_PUBLIC_API_URL : ""}${
            movie.thumbnail
          }`}
          width={355}
          height={370}
      ...
      <div className="w-full flex justify-between items-center">
        <div className="flex items-center mt-2">
          ...
          <p className="font-bold ml-1 text-lg">{movie.imdb} IMDB</p>
        </div>
        <div className="flex items-center mt-2">
          <Icon icon="schedule" variation="text-white" />
          <p className="font-bold ml-1 text-lg">{movie.duration}</p>
        </div>
        <div className="flex items-center mt-2">
          <Icon icon="4k" variation="text-white" />
          <p className="font-bold ml-1 text-lg">4k</p>
        </div>
      </div>
